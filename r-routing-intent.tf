...
resource "null_resource" "routing_precondition" {
  ...
  lifecycle {
    precondition {
      condition     = ((var.azure_firewall_as_next_hop_enabled && var.next_hop_nva_id == null) || (!var.azure_firewall_as_next_hop_enabled && var.next_hop_nva_id != null))
      error_message = "The varialbe 'next_hop_nva_id' must be null if 'azure_firewall_as_next_hop_enabled' is true or must not be emtpy if 'azure_firewall_as_next_hop_enabled' is false."
    }
  }
}
